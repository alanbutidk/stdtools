...
def StubELF(message: str, STUB_ELF: Path) -> None:
    try:
        ...
        if is_64bit:
            shstrtab_offset = struct.unpack(
                f"{end}Q",
                stub_data[shstrtab_offset_hdr + 24 : shstrtab_offset_hdr + 32],
            )[0]
        else:
            shstrtab_offset = struct.unpack(
                f"{end}I",
                stub_data[shstrtab_offset_hdr + 16 : shstrtab_offset_hdr + 20],
            )[0]
        ...
        print(f"\033[32m{out_path} was written!\033[0m")
        print(f'\033[33m"{message}" was stubbed!\033[0m')
...
def StubPE(message: str, STUB_WIN: Path) -> None:
    try:
        ...
        size_of_optional = struct.unpack(
            "<H", stub_data[pe_offset + 20 : pe_offset + 22]
        )[0]
        ...
        for i in range(num_sections):
            ...
            sec_name = (
                stub_data[sh : sh + 8].rstrip(b"\x00").decode("ascii", errors="ignore")
            )
        ...
        stub_data[data_raw_offset : data_raw_offset + len(message_bytes)] = (
            message_bytes
        )
        ...
        with open(out_path, "wb") as f:
            f.write(stub_data)
        print(f"\033[32m{out_path} written!\033[0m")
        print(f'\033[33m"{message}" was stubbed\033[0m')
...
if __name__ == "__main__":
    ...
    try:
        cli.RegisterArg(
            ...
            HelpMsg="The message needed to print.",
        )
    except (ArgTypeError, ValueError):
        print(f"{c.RED}Invalid value provided. Please provide a valid string.{c.RESET}")

    cli.HandleBasic()

    if getattr(sys, "frozen", False) or "__compiled__" in globals():  # pyi or nuitka
        STUB_DIR = Path(sys.executable).parent / "Stubs"
    else:
        STUB_DIR = Path(__file__).parent / "Stubs"  # raw file
    ...
    if cli.print:  # pyright: ignore
        PrintSentence = str(cli.print.value)  # pyright: ignore

    platform = DetectStubPlatform()
    if platform == "ELF":
        StubELF(PrintSentence, STUB_ELF=STUB_ELF)
    else:
        StubPE(PrintSentence, STUB_WIN=STUB_WIN)
